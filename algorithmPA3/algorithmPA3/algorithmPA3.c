...
void writefo2(FILE* fp, BTree* ptree, int const root, const char* prevcode) {

}
int decodeTree(const char* codetext, BTree* tree, int* pt_tree_root) {
	static tree_idx = 0;
	static codetext_idx = 0;
	char ch;
	char curtext[1024] = { 0 };
	int left, right, parent;
	/*while ((ch = codetext[codetext_idx]) != '\0') {
		int left, right;
		switch (ch) {
		case '(':
			codetext_idx++;
			left = decodeTree(codetext, tree, pt_tree_root);
			break;
		default:
			if (myisalpha(ch)) {
				left = tree_idx;
				...
				if (ch == ',') {
					...
					tree[tree_idx].ch = '\0';
					tree[tree_idx].leftc = left;
					tree[tree_idx].rightc = right;
				}
				else if (ch == ')') {
					tree[tree_idx].ch = '\0';
					tree[tree_idx].leftc = left;
	...
	case '(':
		left = decodeTree(codetext, tree, pt_tree_root);
		right = decodeTree(codetext, tree, pt_tree_root);

		parent = tree_idx;
		tree[parent].ch = '\0';
		tree[parent].leftc = left;
		tree[parent].rightc = right;
		tree_idx++;
		break;
	default:
		if (myisalpha(ch)) {
...
int readTree(FILE* fi, FILE* fo) {
	...
	fscanf(fi, "%sHEADEREND", &tree_text);
}
